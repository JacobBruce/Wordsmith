...
bool IsMouseInWindow(wxWindow* win) {
	if (!win || !win->HasFocus()) return false;
...
const wxWindowID WordsmithFrame::ID_MENUITEM57 = wxNewId();
...
const wxWindowID WordsmithFrame::ID_MENUITEM53 = wxNewId();
const wxWindowID WordsmithFrame::ID_MENUITEM54 = wxNewId();
const wxWindowID WordsmithFrame::ID_MENUITEM55 = wxNewId();
const wxWindowID WordsmithFrame::ID_MENUITEM56 = wxNewId();
const wxWindowID WordsmithFrame::ID_MENUITEM58 = wxNewId();
...
bool wsTextCtrl::ReloadFile()
{
	if (m_FilePath.empty() || !FileExists(m_FilePath.utf8_string()))
		return false;
...
void wsDictDialog::FillAutoComplete()
{
	autoWords.Empty();
	autoWords.Alloc(GLOBALS::WordSyn.size());

	for (const auto& entry : GLOBALS::WordSyn)
		autoWords.Add(wxString::FromUTF8(entry.first));
}
	...
	wxDialog(parent, wxID_ANY, caption, pos, sz)
{
	...
	srand(static_cast<unsigned>(wxGetLocalTime()));
...
void wsAboutDialog::CheckButtonClick(wxCommandEvent& event)
{
	...
	if (!webReq.IsOk()) {
		...
		wxMessageBox(_("Could not check for updates."), _("Error"), wxOK|wxICON_ERROR|wxCENTER);
		return;
	}
...
void wsAboutDialog::OnStateChange(wxWebRequestEvent& event)
{
	switch (event.GetState())
	{
		...
		case wxWebRequest::State_Failed:
		{
			checkBtn->SetLabel(_("Check for Update"));
			checkBtn->Enable(true);
			wxString err = event.GetErrorDescription();
			if (err.empty())
				err = _("Could not check for updates.");
			wxMessageBox(err, _("Error"), wxOK|wxICON_ERROR|wxCENTER);
			break;
		}
	}
}
...
void HTMLFrame::ReloadPage()
{
	if (viewMode == 0) {
		...
		if (errStr.empty()) {
			std::string htmlStr = "<!DOCTYPE html><html><head><meta charset='utf-8'><base href='" + EncodeUriPath(fileDir) +
			...
			if (errStr.empty())
				webView->LoadURL(wxFileName::FileNameToURL(tmpFile));
		} else {
			wxMessageBox(errStr, _("Error"), wxOK|wxICON_ERROR|wxCENTER);
		}
	} else {
		webView->LoadURL(wxFileName::FileNameToURL(filePath));
	}
}
	...
	easyPrint("Printing", this)
{
	...
	ReloadMenuItem = new wxMenuItem(FileMenu, ID_MENUITEM57, _("Reload From Disk"), _("Reload the current file from disk"), wxITEM_NORMAL);
	...
	ToolsMenu->AppendSeparator();
	CaseMenuItem = new wxMenu();
	CaseMenuItem->Append(ID_MENUITEM53, _("Uppercase"), wxEmptyString, wxITEM_NORMAL);
	CaseMenuItem->Append(ID_MENUITEM54, _("Lowercase"), wxEmptyString, wxITEM_NORMAL);
	CaseMenuItem->AppendSeparator();
	CaseMenuItem->Append(ID_MENUITEM55, _("Capitalize"), wxEmptyString, wxITEM_NORMAL);
	CaseMenuItem->Append(ID_MENUITEM56, _("Uncapitalize"), wxEmptyString, wxITEM_NORMAL);
	ToolsMenu->Append(ID_MENUITEM58, _("Case"), CaseMenuItem, _("Change letter case"));
	...
	Connect(ID_MENUITEM57, wxEVT_COMMAND_MENU_SELECTED, (wxObjectEventFunction)&WordsmithFrame::OnReloadClick);
	...
	Connect(ID_MENUITEM53, wxEVT_COMMAND_MENU_SELECTED, (wxObjectEventFunction)&WordsmithFrame::OnCaseClick);
	Connect(ID_MENUITEM54, wxEVT_COMMAND_MENU_SELECTED, (wxObjectEventFunction)&WordsmithFrame::OnCaseClick);
	Connect(ID_MENUITEM55, wxEVT_COMMAND_MENU_SELECTED, (wxObjectEventFunction)&WordsmithFrame::OnCaseClick);
	Connect(ID_MENUITEM56, wxEVT_COMMAND_MENU_SELECTED, (wxObjectEventFunction)&WordsmithFrame::OnCaseClick);
	...
	for (const auto& pathPair : GLOBALS::ArgFiles)
		OpenFilePath(pathPair.first);

	if (GLOBALS::TabPageSTC == nullptr) {
		NewPadTab();
	} else if (GLOBALS::ArgFiles.empty() && !selFile.empty()) {
...
void WordsmithFrame::ShowOpenDialog()
{
	...
	OpenFilePath(FileOpenDialog1->GetPath());
}

void WordsmithFrame::OpenFilePath(const wxString& file_path)
{
	wxFileName fn = file_path.StartsWith("file:")
		? wxFileName::URLToFileName(file_path)
		: wxFileName(file_path);

	if (!fn.IsAbsolute())
		fn.MakeAbsolute();

	const wxString filePath = fn.GetFullPath();

	if (!FileExists(filePath.utf8_string())) {
	...
	try {
		...
		const wxString fileName = fn.GetFullName();
...
bool WordsmithFrame::ShowSaveDialog(bool force_show)
{
	if (GLOBALS::TabPageSTC == nullptr) return false;

	try {
		if (force_show || GLOBALS::TabPageSTC->GetFilePath().empty()) {
			if (FileSaveDialog1->ShowModal() == wxID_CANCEL) return false;
...
{
	wxFileName want(file_name);

	if (!want.IsAbsolute())
		want.MakeAbsolute();

	const wxString wantPath = want.GetFullPath();
	...
	for (size_t i=0; i < AuiNotebook1->GetPageCount(); ++i)
	{
		...
		wxFileName have(stc->GetFilePath());

		if (have.GetFullPath().empty()) continue;

		if (!have.IsAbsolute())
			have.MakeAbsolute();

		if (wantPath == have.GetFullPath()) {
...
void WordsmithFrame::LoadDocument(const wxString& file_path, const wxString& file_name)
{
	...
	GLOBALS::TabPageSTC->SetSavePoint();
...
{
	if (pageIdx >= AuiNotebook1->GetPageCount()) return;

	wsTextCtrl* stc = GetTextCtrlForPage(AuiNotebook1->GetPage(pageIdx));
	if (!stc) return;

	wxString tip = stc->GetFilePath();
	if (tip.empty())
		tip = _("New File");

	AuiNotebook1->SetPageToolTip(pageIdx, tip);
}
...
void WordsmithFrame::CheckFileState()
{
	...
	if (GLOBALS::TabPageSTC->GetFilePath().empty()) return;
...
void WordsmithFrame::BindTextCtrl(wsTextCtrl* stc)
{
	...
	stc->Bind(wxEVT_STC_UPDATEUI, &WordsmithFrame::OnUpdateUI, this);
	stc->Bind(wxEVT_STC_CLIPBOARD_PASTE, &WordsmithFrame::OnClipboardPaste, this);
	stc->Bind(wxEVT_STC_SAVEPOINTREACHED, &WordsmithFrame::OnSavePointReached, this);
	stc->Bind(wxEVT_STC_SAVEPOINTLEFT, &WordsmithFrame::OnSavePointLeft, this);
...
void WordsmithFrame::NewPadTab(int wrap, int zoom)
{
	...
	UpdateTabToolTip(AuiNotebook1->GetPageCount() - 1);
}
...
bool WordsmithFrame::CanSaveDocument() const
{
	// Untitled tabs have no path yet; Save must stay enabled so Ctrl+S can
	// fall through to Save As. Named files only need Save when dirty.
	return GLOBALS::TabPageSTC &&
		   (GLOBALS::TabPageSTC->GetFilePath().empty() ||
			GLOBALS::TabPageSTC->IsChanged());
}

void WordsmithFrame::UpdateTabSaveState(wsTextCtrl* stc, bool changed)
{
	if (!stc) return;

	stc->SetChanged(changed);

	wxWindow* panel = stc->GetParent();
	const int pageIdx = panel ? AuiNotebook1->GetPageIndex(panel) : wxNOT_FOUND;

	if (pageIdx != wxNOT_FOUND) {
		const wxString name = stc->GetFileName();
		AuiNotebook1->SetPageText(pageIdx, changed ? "*" + name : name);
	}

	if (stc == GLOBALS::TabPageSTC)
		UpdateToolBar();
}

void WordsmithFrame::OnSavePointReached(wxStyledTextEvent& event)
{
	...
	CallAfter([this, stc]() {
		for (size_t i = 0; i < AuiNotebook1->GetPageCount(); ++i) {
			if (GetTextCtrlForPage(AuiNotebook1->GetPage(i)) == stc) {
				UpdateTabSaveState(stc, false);
				return;
			}
		}
	});
...
void WordsmithFrame::OnSavePointLeft(wxStyledTextEvent& event)
{
	wsTextCtrl* stc = wxDynamicCast(event.GetEventObject(), wsTextCtrl);
	event.Skip();
	if (!stc) return;
...
void WordsmithFrame::UpdateToolBar()
{
	...
	SaveMenuItem->Enable(canSave);

	if (GLOBALS::TabPageSTC != nullptr) {
		ToolBar1->EnableTool(SaveAsToolBarBtn->GetId(), true);
		ToolBar1->EnableTool(SaveToolBarBtn->GetId(), canSave);
		...
		ToolBar1->EnableTool(PasteToolBarBtn->GetId(), true);
...
void WordsmithFrame::OnSessionStatsClick(wxCommandEvent& event)
{
	...
	ss << _("Chars per minute: ") << std::setprecision(3) << (sessMins > 0.f ? charsTyped / sessMins : 0.f);
...
void WordsmithFrame::OnReloadClick(wxCommandEvent& event)
{
	if (GLOBALS::TabPageSTC == nullptr) return;

	wsTextCtrl& stc(*GLOBALS::TabPageSTC);
	if (stc.GetFilePath().empty()) return;

	if (stc.AutoCompActive())
		stc.AutoCompCancel();

	if (!FileExists(stc.GetFilePath().utf8_string())) {
		ErrorDialog1->SetMessage(_("Failed to reload file: ") + stc.GetFilePath());
		ErrorDialog1->ShowModal();
		return;
	}

	if (stc.IsChanged()) {
		QuestionDialog1->SetMessage(_("Reload the file from disk and discard unsaved changes?"));
		if (QuestionDialog1->ShowModal() != wxID_YES)
			return;
	}
	...
	AuiNotebook1->SetPageText(AuiNotebook1->GetSelection(), stc.GetFileName());
	UpdateTabToolTip(AuiNotebook1->GetSelection());
...
void WordsmithFrame::OnPasteClick(wxCommandEvent& event)
{
	...
	if (GLOBALS::TabPageSTC->CanPaste()) {
		...
		MessageDialog1->SetMessage(_("The clipboard is empty or does not contain text."));
		MessageDialog1->ShowModal();
	}
}
...
void WordsmithFrame::OnCaseClick(wxCommandEvent& event)
{
	if (GLOBALS::TabPageSTC == nullptr) return;
	wsTextCtrl& stc(*GLOBALS::TabPageSTC);

	if (stc.GetSelectionEmpty()) return;

	const int startPos = stc.GetSelectionStart();
	const int endPos = stc.GetSelectionEnd();

	if (startPos >= endPos) return;

	wxString text(stc.GetTextRange(startPos, endPos));
	const int id = event.GetId();

	if (id == ID_MENUITEM53) {
		WordParser::UpperString(text);
	} else if (id == ID_MENUITEM54) {
		WordParser::LowerString(text);
	} else if (id == ID_MENUITEM55) {
		WordParser::UpperFirst(text);
	} else if (id == ID_MENUITEM56) {
		WordParser::LowerFirst(text);
	} else {
		return;
	}

	stc.Replace(startPos, endPos, text);
...
void WordsmithFrame::OnTextChanged(wxStyledTextEvent& event)
{
	...
	const int modType = event.GetModificationType();
	const bool fromUndoRedo = (modType & (wxSTC_PERFORMED_UNDO | wxSTC_PERFORMED_REDO)) != 0;

	// Completing an autocomplete item inserts the rest of the word (length > 1).
	// Only treat clipboard paste as a bulk insert that should dismiss AC.
	if (suppressAutoComp && !fromUndoRedo && stc->AutoCompActive() &&
		(modType & (wxSTC_MOD_INSERTTEXT | wxSTC_MOD_DELETETEXT))) {
		CallAfter([this]() {
			if (GLOBALS::TabPageSTC && GLOBALS::TabPageSTC->AutoCompActive())
				GLOBALS::TabPageSTC->AutoCompCancel();
		});
	}
...
void WordsmithFrame::OnUpdateUI(wxStyledTextEvent& event)
{
	if ((event.GetUpdated() & wxSTC_UPDATE_V_SCROLL) && SpellCheckMenuItem->IsChecked())
		if (!spellTimer.IsRunning()) spellTimer.Start(200, true);

	event.Skip();
}

void WordsmithFrame::OnClipboardPaste(wxStyledTextEvent& event)
{
	suppressAutoComp = true;
	event.Skip();
}
...
{
	wsTextCtrl* stc = wxDynamicCast(event.GetEventObject(), wsTextCtrl);
	if (!stc || !GLOBALS::GotWordData) return;
...
void WordsmithFrame::OnTabPageClose(wxAuiNotebookEvent& event)
{
	...
	if (stc->IsChanged() && (!stc->GetFilePath().empty() || stc->GetLength() > 0)) {
		...
		wxMessageDialog savePrompt(this,
			_("Do you want to save the document before closing it?"),
			_("Question"),
			wxYES_NO|wxCANCEL|wxYES_DEFAULT|wxICON_QUESTION);
		const int answer = savePrompt.ShowModal();
		...
		if (answer == wxID_YES && !ShowSaveDialog(stc->GetFilePath().empty())) {
			event.Veto();
			return;
		}
	}
...
void WordsmithFrame::OnSpellCheckTrigger(wxTimerEvent& event)
{
	...
	const int docLen = stc.GetTextLength();
	const int firstLine = stc.DocLineFromVisible(stc.GetFirstVisibleLine());
	int lastVisible = stc.GetFirstVisibleLine() + stc.LinesOnScreen();
	if (lastVisible < 0) lastVisible = 0;
	int lastLine = stc.DocLineFromVisible(lastVisible);
	const int maxLine = stc.GetLineCount();
	if (lastLine < firstLine) lastLine = firstLine;
	if (lastLine >= maxLine) lastLine = maxLine - 1;

	auto fillIndicator = [&](int start, int end) {
		if (start < 0 || end <= start || end > docLen) return;
		stc.IndicatorFillRange(start, end - start);
	};

	for (int line = firstLine; line <= lastLine; ++line)
	{
		int curPos = stc.PositionFromLine(line);
		const int endPos = stc.GetLineEndPosition(line);

		if (curPos >= endPos) continue;

		stc.IndicatorClearRange(curPos, endPos - curPos);
		...
		while (curPos < endPos)
		{
			const int nextPos = stc.PositionAfter(curPos);
			if (nextPos <= curPos) break;

			const wxString chStr = stc.GetTextRange(curPos, nextPos);
			const wxUniChar c = chStr.empty() ? wxUniChar(0) : chStr[0];
...
void WordsmithFrame::OnACKeyTimerTrigger(wxTimerEvent& event)
{
	...
	static std::string lastTopWord;
	static wxString preChars;
	static uint32_t wordIndex;
	static int wordPos;
	static bool nextWordList = false;

	constexpr size_t kNoScrollbar = 5;

	if (!GLOBALS::GotWordData || GLOBALS::TabPageSTC == nullptr) return;
	if (inAutoCompUpdate) return;

	if (suppressAutoComp) {
		suppressAutoComp = false;
		return;
	}

	struct InACUpdate {
		bool& flag;
		explicit InACUpdate(bool& f) : flag(f) { flag = true; }
		~InACUpdate() { flag = false; }
	} inAC(inAutoCompUpdate);
	...
	const bool wasActive = stc.AutoCompActive();

	auto collectMatches = [&]()
	{
		subWord = preChars.length() >= 2 ? preChars.substr(0, 2).utf8_string() : preWord;
		wordPos = GLOBALS::UserWords.size();

		auto scan = [&](const std::vector<std::pair<std::string, uint32_t>>& vec,
						const phmap::parallel_flat_hash_map<std::string, uint32_t>& indices)
		{
			uint32_t startIdx = vec.size();

			if (preChars.length() >= 2) {
				if (!indices.contains(subWord)) return;
				startIdx = indices.at(subWord);
			} else {
				for (const auto& entry : indices) {
					if (!entry.first.empty() && entry.first[0] == subWord[0])
						startIdx = std::min(startIdx, entry.second);
				}
				if (startIdx >= vec.size()) return;
			}

			for (wordIndex = startIdx; wordIndex < vec.size(); ++wordIndex)
			{
				const auto& entry(vec[wordIndex]);

				if (preChars.length() >= 2) {
					if (!entry.first.starts_with(subWord)) break;
				} else if (!entry.first.starts_with(preWord)) {
					break;
				}

				if (entry.first.length() <= preWord.length() || !entry.first.starts_with(preWord)) continue;
		...
		scan(GLOBALS::WordVec, GLOBALS::WordIndices);
		scan(GLOBALS::ExtraVec, GLOBALS::ExtraIndices);

		for (const std::string& word : GLOBALS::UserWords)
		{
			if (word.length() <= preWord.length() || !word.starts_with(preWord)) continue;
			autoWords.emplace(std::make_pair(wordPos--, word));
		}
	};

	auto showWordList = [&](int lenEntered)
	{
		if (autoWords.empty()) {
			lastTopWord.clear();
			return;
		...
		stc.AutoCompShow(lenEntered, wxString::FromUTF8(TrimLast(ss.str(), '\n')));
		lastTopWord = autoWords.begin()->second;
	};
	...
	if (GLOBALS::GotPhraseKeys) {
		...
		if (!autoPhrases.empty())
			stc.AutoCompShow(enteredBytes, wxString::FromUTF8(TrimLast(ss.str(), '\n')));

		GLOBALS::ManualAutoComp = false;
		return;
	}

	if (!GLOBALS::ManualAutoComp && GLOBALS::LastKeyPress == WXK_SPACE && preWord.empty()) {

		wordPos = std::max(0, stc.GetCurrentPos() - 2);

		if (WordParser::IsEnd2(stc.GetCharAt(stc.GetCurrentPos()-1), stc.GetCharAt(wordPos))) {
			GLOBALS::ManualAutoComp = false;
			return;
		}
		...
		if (preWord.empty()) {
			...
			if (!GLOBALS::WordMap.contains(preWord)) {
				GLOBALS::ManualAutoComp = false;
				return;
			}
		}
	...
	if ((wasActive || nextWordList) && GLOBALS::LastKeyPress != WXK_SPACE) {

		for (auto it = autoWords.begin(); it != autoWords.end(); ) {
			if (it->second.length() <= preWord.length() || !it->second.starts_with(preWord))
				it = autoWords.erase(it);
			else
				++it;
		}

		for (auto it = autoPhrases.begin(); it != autoPhrases.end(); ) {
			if (it->length() < preWord.length() || !it->starts_with(preWord))
				it = autoPhrases.erase(it);
			else
				++it;
		}

		if (!autoWords.empty() && autoWords.begin()->second == lastTopWord)
			autoWords.erase(autoWords.begin());

		if (!autoPhrases.empty() && *autoPhrases.begin() == lastTopWord)
			autoPhrases.erase(autoPhrases.begin());

		if (!autoPhrases.empty()) {
			std::stringstream ss;
			for (const std::string& phrase : autoPhrases) ss << phrase << "\n";
			stc.AutoCompShow(enteredBytes, wxString::FromUTF8(TrimLast(ss.str(), '\n')));
			lastTopWord = *autoPhrases.begin();
			GLOBALS::ManualAutoComp = false;
			return;
		}

		if (autoWords.size() <= kNoScrollbar) {
			nextWordList = false;
			autoWords.clear();
			collectMatches();
		}

		showWordList(enteredBytes);
		GLOBALS::ManualAutoComp = false;
		return;
	}
...
void WordsmithFrame::OnKeyPress(wxKeyEvent& event)
{
	...
	if (event.GetUnicodeKey() != WXK_NONE && !event.ControlDown() && !event.AltDown()) {
		...
		if (suppressAutoComp) {
			suppressAutoComp = false;
		} else if (GLOBALS::AutoCompMode && !ackeyTimer.IsRunning()) {
			ackeyTimer.Start(10, true);
		}
...
static int ToSTCFindFlags(int flags)
{
	int stcFlags = wxSTC_FIND_NONE;
...
void WordsmithFrame::FindNext(const wxString& find_str, const int& flags)
{
	...
	const int stcFlags = ToSTCFindFlags(flags);
	const int docLen = textCtrl.GetTextLength();
	const bool down = (flags & wxFR_DOWN) != 0;

	textCtrl.SetSearchFlags(stcFlags);

	if (down) {
		textCtrl.SetTargetStart(textCtrl.GetSelectionEnd());
		textCtrl.SetTargetEnd(docLen);
	} else {
		textCtrl.SetTargetStart(textCtrl.GetSelectionStart());
		textCtrl.SetTargetEnd(0);
	}

	findPos = textCtrl.SearchInTarget(find_str);
	...
	if (findPos == wxSTC_INVALID_POSITION) {
		if (down) {
			...
			textCtrl.SetTargetStart(docLen);
			textCtrl.SetTargetEnd(0);
		}
		findPos = textCtrl.SearchInTarget(find_str);
	}
	...
	const int start = textCtrl.GetTargetStart();
	const int end = textCtrl.GetTargetEnd();
	findPos = start;

	if (down) {
		textCtrl.SetAnchor(start);
		textCtrl.SetInsertionPoint(end);
		textCtrl.ScrollToColumn(textCtrl.GetColumn(start));
	} else {
		textCtrl.SetAnchor(end);
		textCtrl.SetInsertionPoint(start);
		textCtrl.ScrollToColumn(textCtrl.GetColumn(end));
	}
...
void WordsmithFrame::OnFindReplace(wxFindDialogEvent& event)
{
	...
	const int selStart = textCtrl.GetSelectionStart();
	const int selEnd = textCtrl.GetSelectionEnd();
	bool selectionIsMatch = false;

	if (selStart != selEnd) {
		textCtrl.SetSearchFlags(ToSTCFindFlags(event.GetFlags()));
		textCtrl.SetTargetStart(selStart);
		textCtrl.SetTargetEnd(selEnd);
		selectionIsMatch = textCtrl.SearchInTarget(findStr) == selStart &&
						   textCtrl.GetTargetEnd() == selEnd;
	}

	if (!selectionIsMatch) {
		FindNext(findStr, event.GetFlags());
...
void WordsmithFrame::OnFindReplaceAll(wxFindDialogEvent& event)
{
	...
	stc.SetSearchFlags(ToSTCFindFlags(event.GetFlags()));
	stc.BeginUndoAction();

	size_t repNum = 0;
	stc.SetTargetStart(0);
	stc.SetTargetEnd(stc.GetTextLength());

	while (stc.SearchInTarget(findStr) != wxSTC_INVALID_POSITION)
	{
		stc.ReplaceTarget(replaceStr);
		++repNum;

		const int next = stc.GetTargetEnd();
		stc.SetTargetStart(next);
		stc.SetTargetEnd(stc.GetTextLength());
	}

	stc.EndUndoAction();

	wxString msg = _("Replaced %u occurrences");
	msg.Replace("%u", std::to_string(repNum), true);
...
void WordsmithFrame::OnMenuOpen(wxMenuEvent& event)
{
	...
	if (menu->GetTitle() == _("&File")) {
		SaveMenuItem->Enable(CanSaveDocument());
		SaveAsMenuItem->Enable(tb);
		ReloadMenuItem->Enable(tb && !GLOBALS::TabPageSTC->GetFilePath().empty());
		...
		if (tb && !GLOBALS::TabPageSTC->GetSelectionEmpty())
			caseEnabled = true;

		if (wxMenuItem* caseParent = ToolsMenu->FindItem(ID_MENUITEM58))
			caseParent->Enable(caseEnabled);

		wxMenuItemList& caseItems(CaseMenuItem->GetMenuItems());
		for (wxMenuItem* menuItem : caseItems) {
			if (!menuItem->IsSeparator())
				menuItem->Enable(caseEnabled);
		}
...
void WordsmithFrame::ShowEditorContextMenu(wsTextCtrl& stc, const wxPoint& clientPos)
{
	...
	if (!stc.GetSelectionEmpty()) {
		...
		caseMenu->Append(ID_MENUITEM53, _("Uppercase"));
		caseMenu->Append(ID_MENUITEM54, _("Lowercase"));
		caseMenu->AppendSeparator();
		caseMenu->Append(ID_MENUITEM55, _("Capitalize"));
		caseMenu->Append(ID_MENUITEM56, _("Uncapitalize"));
		caseMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, &WordsmithFrame::OnCaseClick, this, ID_MENUITEM53);
		caseMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, &WordsmithFrame::OnCaseClick, this, ID_MENUITEM54);
		caseMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, &WordsmithFrame::OnCaseClick, this, ID_MENUITEM55);
		caseMenu->Bind(wxEVT_COMMAND_MENU_SELECTED, &WordsmithFrame::OnCaseClick, this, ID_MENUITEM56);
		menu.Append(wxNewId(), _("Case"), caseMenu);
		addSep = true;
	}
...
{
	// Do not AutoCompCancel here. Showing the list steals focus from the
	// editor; cancelling on KILL_FOCUS dismisses next-word suggestions
	// (auto-space) and can lock GTK if it runs during AutoCompShow.
	event.Skip();
}
...
void WordsmithFrame::OnActivate(wxActivateEvent& event)
{
	if (event.GetActive()) {
		CheckFileState();
	} else if (GLOBALS::TabPageSTC && GLOBALS::TabPageSTC->AutoCompActive()) {
		GLOBALS::TabPageSTC->AutoCompCancel();
	}
